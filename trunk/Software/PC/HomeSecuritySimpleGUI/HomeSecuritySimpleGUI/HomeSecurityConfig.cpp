...
bool HomeSecurityConfig::loadConf(LPTSTR path)
{
	CXMLFile xml_file;
	...
	if( !parseConf(&xml_file, device_list) )
...
bool HomeSecurityConfig::parseConf(CXMLFile* xml_file, list<Device*>& devices)
{
...
bool HomeSecurityConfig::programConf(list<Device*>& devices, int port, int baudRate)
{
	char data[3];
...
list<Device*> HomeSecurityConfig::getDeviceList()
{
	return device_list;
}

void HomeSecurityConfig::sendByteSerial(unsigned char byte)
{
	s->SendData((const char*)&byte, 1);
}